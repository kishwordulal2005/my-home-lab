...
def filter_xss(value, level):
    """
    XSS filter — applied to user input before rendering in templates.

    easy:   No filtering.
    medium: Strip <script> tags only (regex). Other tags pass through.
    hard:   Strip <script> tags + common event handler attributes.
            Deliberately beaten by rare tags/attributes and broken/nested tags.
    """
    if level == "easy":
        return value

    if level == "medium":
        # Strip <script> opening/closing tags (case-insensitive)
        result = re.sub(r'<\s*script[^>]*>', '', value, flags=re.IGNORECASE)
        result = re.sub(r'<\s*/\s*script\s*>', '', result, flags=re.IGNORECASE)
        return result
    ...
    if level == "hard":
        ...
        result = re.sub(r'<\s*script[^>]*>', '', value, flags=re.IGNORECASE)
        result = re.sub(r'<\s*/\s*script\s*>', '', result, flags=re.IGNORECASE)
        # Strip common event handler attributes from all tags (case-sensitive!)
        # This is deliberately case-sensitive so <OnErRor> slips past.
        event_handlers = [
        ...
        for handler in event_handlers:
            # Only match lowercase — case variation beats this
            result = re.sub(
                rf'\s+{handler}\s*=\s*["\'][^"\']*["\']',
                '', result
            )
...
def filter_dom_xss(value, level):
    """
    DOM-specific sanitizer — wraps the naive "sanitize" that only strips
    exact <script> substrings. Applied client-side in JS, but we provide
    the function here for reference / server-side echo of the same logic.
    """
    if level == "easy":
        return value
    if level == "medium":
    ...
    if level == "hard":
        # Only strips exact "<script>" substring — beaten by "<scr<script>ipt>"
        # or mixed case or encoded variants
